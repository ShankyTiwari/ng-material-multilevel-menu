  ...
  implements OnInit, OnChanges, OnDestroy
{
  ...
  ngOnInit() {
    if (
      !CommonUtils.isNullOrUndefinedOrEmpty(this.configuration) &&
  ...
  updateNodeByURL(url: string): void {
    ...
    if (
      ...
      if (
        !CommonUtils.isNullOrUndefined(foundNode.dontEmit) &&
        !foundNode.dontEmit
      ) {
  ...
  detectInvalidConfig(): void {
    if (CommonUtils.isNullOrUndefinedOrEmpty(this.configuration)) {
      ...
      if (
        !CommonUtils.isNullOrUndefined(config.paddingAtStart) &&
      ...
      if (
        !CommonUtils.isNullOrUndefined(config.interfaceWithRoute) &&
        typeof config.interfaceWithRoute === "boolean"
      ) {
        this.nodeConfig.interfaceWithRoute = config.interfaceWithRoute;
      }
      if (
        !CommonUtils.isNullOrUndefined(config.collapseOnSelect) &&
        typeof config.collapseOnSelect === "boolean"
      ) {
        this.nodeConfig.collapseOnSelect = config.collapseOnSelect;
      }
      if (
        !CommonUtils.isNullOrUndefined(config.highlightOnSelect) &&
        typeof config.highlightOnSelect === "boolean"
      ) {
        this.nodeConfig.highlightOnSelect = config.highlightOnSelect;
      }
      if (
        !CommonUtils.isNullOrUndefined(config.useDividers) &&
        typeof config.useDividers === "boolean"
      ) {
        this.nodeConfig.useDividers = config.useDividers;
      }
      if (
        !CommonUtils.isNullOrUndefined(config.rtlLayout) &&
        typeof config.rtlLayout === "boolean"
      ) {
        this.nodeConfig.rtlLayout = config.rtlLayout;
      }
      if (
        !CommonUtils.isNullOrUndefined(config.customTemplate) &&
  ...
  initExpandCollapseStatus(): void {
    this.expandCollapseStatusSubscription =
      this.multilevelMenuService.expandCollapseStatus$.subscribe(
  ...
  initSelectedMenuID(): void {
    this.selectMenuByIDSubscription =
      this.multilevelMenuService.selectedMenuID$.subscribe(
        (selectedMenuID: string) => {
          if (selectedMenuID) {
            const foundNode = this.multilevelMenuService.getMatchedObjectById(
              this.items,
              selectedMenuID
            );
  ...
  getGlobalStyle(): BackgroundStyle {
    if (!this.isInvalidConfig) {
      ...
      if (
        !CommonUtils.isNullOrUndefinedOrEmpty(
